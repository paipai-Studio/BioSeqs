...
| `swfdr.mbt` / `swfdr_lbfgs.mbt` | Bioconductor swfdr：swfdr_lm_pi0 协变量调节 π₀（λ 网格 Storey 估计 + logistic/linear 回归、unit.spline 等距三次样条与 smooth.spline（df=3，交叉验证折数）两种 pi0_smooth、threshold [0,1] 截断）、swfdr_lm_qvalue（pFDR 有限样本校正、单调化、pfdr 开关）、swfdr_calculate science-wise EM（rounded 六粗粒度箱 prob0=(.1,.2,.2,.2,.2,.1) 与 truncated P(p≤观测) 似然、Beta(α,β) 备择、z 后验零假设指示 rounded→None、n0/n 箱计数、自定义初值/迭代数）；`swfdr_lbfgs.mbt` 完整 L-BFGS-B v2.3（m=5/factr=1e7，BLAS ddot/dcopy/daxpy/dscal、Linpack dpofa/dtrsl、cauchy/freev/formk/formt/matupd/subsm、Moré-Thuente dcstep/dcsrch 线搜索、环形缓冲、越界截断中心差分 ndeps=1e-3） |
...
| `entrez_eutils.mbt` | Bio.Entrez 全套 9 个 E-utilities：einfo + parse_einfo（无 db 返回 DbList 数据库列表；有 db 解析 MenuName/Description/Count/LastUpdate 与 FieldList/LinkList 名称，20190110 DTD）、esearch + parse_esearch（Count/RetMax/RetStart/IdList、QueryTranslation 术语翻译、usehistory 的 QueryKey/WebEnv）、efetch（rettype/retmode 返回原始记录文本、query_key/WebEnv history 拉取、≥200 ID 自动 POST）、epost（WebEnv/QueryKey 会话）、esummary + parse_esummary（DocSum/Item 文档摘要）、espell + parse_espell（CorrectedQuery/Replaced）、elink + parse_elink（LinkSet/LinkSetDb/LinkName 跨库关联）、egquery + parse_egquery（Term/Result 全局跨库计数、EGQueryResult::get 查询）、ecitmatch + parse_ecitmatch（journal|year|vol|page|author 引文 bdata 检索 PMID）、entrez_quote_plus URL 编码、EntrezClient 可注入 fetch 与 now 时钟、内置限速（无 API key 1/3 秒、有 key 1/11 秒） |
