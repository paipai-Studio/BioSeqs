...
| **DESeq2** | Bioconductor DESeq2 | 差异表达分析、负二项分布模型 | ✅ |
...
| **ChIPseeker** | Bioconductor ChIPseeker | ChIP-seq峰注释、基因距离计算、注释分类（启动子/外显子/内含子/基因间区）、结果汇总与可视化 | ✅ |
...
| **DiffBind** | Bioconductor DiffBind | ChIP-seq差异结合分析、峰值重叠、共识峰识别、TMM归一化、负二项分布检验 | ✅ |
...
| **GSVA** | Bioconductor GSVA | 基因集变异分析、单样本通路评分（ssGSEA/zscore/PLAGE）、富集分析、置换检验 | ✅ |
...
| **ChIPseeker** | Bioconductor ChIPseeker | ChIP-seq峰值注释、基因组区域分类、距离TSS分布、注释可视化、统计分析 | ✅ |
...
│   ├── chipseeker.mbt          # ChIPseeker ChIP-seq峰注释分析 (峰-基因距离计算、注释分类、结果汇总)
...
│   ├── gsva.mbt                 # GSVA 基因集变异分析 (ssGSEA/zscore/PLAGE评分、富集分析、置换检验)
...
│   ├── chipseeker.mbt           # ChIPseeker ChIP-seq峰值注释 (基因组区域分类、距离TSS分布、注释可视化、统计分析)
│   └── utils.mbt               # 通用工具函数
├── examples/                   # 示例程序
...
│   ├── bioconductor_demo/      # Bioconductor模块综合示例 (ChIPseeker峰注释、DOSE疾病富集、ReactomePA通路分析、AnnotationDbi注释数据库、clusterProfiler富集框架、WGCNA共表达网络)
...
│   ├── gsva_demo/              # GSVA 基因集变异分析示例 (ssGSEA/zscore/PLAGE评分、富集分析、置换检验)
...
│   └── chipseeker_demo/         # ChIPseeker ChIP-seq峰值注释示例 (基因组区域分类、距离TSS分布、注释可视化、统计分析)
├── test/
...
| MoonBit 文件 | 对应 Python 库 | 核心功能 |
| :--- | :--- | :--- |
| `deseq2.mbt` | Bioconductor DESeq2 | 负二项分布差异表达 |
...
| `chipseeker.mbt` | Bioconductor ChIPseeker | ChIP-seq峰注释分析 |
...
| `chipseeker.mbt` | Bioconductor ChIPseeker | ChIP-seq峰值注释 (基因组区域分类、距离TSS分布、注释可视化) |
...
实现 ChIP-seq 峰注释功能，支持将峰区域与基因特征进行关联分析。可以计算峰到转录起始位点（TSS）的距离，将峰分类为启动子区、外显子区、内含子区或基因间区。提供注释结果汇总统计和可视化功能，便于分析峰在基因组上的分布特征。支持自定义启动子区域范围和注释分类标准。
...
实现 ChIP-seq 峰值注释和基因组区域分析功能，支持峰值到基因的关联和可视化。可以创建 Peak 对象（染色体、起始/终止位置、峰值 ID）和 PeakAnnotation 对象（注释信息、基因名、到 TSS 的距离、功能类型、链）。支持基于 TxDb 的峰值注释（annotate_peaks），自动查找最近基因并计算到 TSS 的距离。基因组区域分类包括：启动子（<=0bp、0-2kb、2-5kb）、远端区域（5-10kb、10-50kb）和基因间区域（>50kb）。支持注释分布条形图（plot_annotation_bar）和距离 TSS 分布图（plot_dist_to_tss）的 ASCII 可视化。支持统计分析，包括各区域百分比计算和按功能类型过滤（get_promoter_annotations、get_distal_annotations、get_intergenic_annotations）。支持注释结果汇总（annotation_summary）。适用于 ChIP-seq 数据分析和峰值功能注释。
...
| ChIPseeker | `chipseeker_test.mbt` | 5 |
...
| ChIPseeker | `chipseeker_test.mbt` | 10 |
...
| bioconductor_demo | Bioconductor模块综合示例（ChIPseeker峰注释、DOSE疾病富集、ReactomePA通路分析、AnnotationDbi注释数据库、clusterProfiler富集框架、WGCNA共表达网络） | `moon run examples/bioconductor_demo/main.mbt` |
...
| gsva_demo | GSVA 基因集变异分析（ssGSEA/zscore/PLAGE评分、富集分析、置换检验、统计摘要） | `moon run examples/gsva_demo/main.mbt` |
...
| chipseeker_demo | ChIPseeker ChIP-seq峰值注释（基因组区域分类、距离TSS分布、注释可视化、统计分析） | `moon run examples/chipseeker_demo/main.mbt` |
