...
| **CSI 索引** | htslib | CSI（Coordinate Sorted Index，VCF/BCF 的伴侣索引）：`parse_csi`（magic `CSI\1` + min_shift/n_lvls/l_aux + 每参考序列 `bin/loffset/n_chunk/chunks`，末尾 `n_no_coor`，无线性索引）+ `csi_reg2bins`（区间→bin 映射，按 min_shift/depth 泛化的 htslib `hts_reg2bins`） |
...
│   └── ... （更多 examples/*_demo/）
├── test/
│   ├── moonbit/                      # MoonBit 单元测试（533 个测试文件，13680 用例）
...
```bash
moon build                                              # ✅ 成功
moon test                                               # ✅ 13680 个测试全部通过（test/moonbit 包）
```
...
```bash
moon build      # 构建项目
moon test       # 运行全部测试 (13680 个测试用例，test/moonbit 包)
```
