...
```
moon build                                              # ✅ 成功
moon test --package IvanAXu/BioSeqs/test/moonbit        # ✅ 2172 个测试全部通过
```
...
```moonbit
// GC 滑动窗口偏斜分析
let gc_values = @src.gc_skew("ATGCGCGCGCGCGCGCGCGCGA", 8, 1)
// gc_values = [0.5, 0.33, 0.25, ...]

// AT 滑动窗口偏斜分析
let at_values = @src.at_skew("ATATATCGCGCGCGCGA", 8, 1)

// ORF 预测
let orfs = @src.find_orfs("ATGCGATCGATCGATCGTA")
// orfs = [{frame: 0, start: 0, end: 15, length: 15, ...}]

// Hamming 距离
let dist = @src.hamming_distance("ACTG", "ACTG")
// dist = 0

// Levenshtein 编辑距离
let edit_dist = @src.edit_distance("kitten", "sitting")
// edit_dist = 3
```
...
// 计算每个位置的信息含量
let ic_per_pos = @src.motif_per_position_ic(motif)
// ic_per_pos = [1.65, 1.32, 1.56]

// 计算总信息含量
let total_ic = @src.motif_total_ic(motif)
// total_ic = 4.53

// 生成序列 Logo 数据
let sequences = ["ACGT", "ACGT", "ACGT", "TCGT"]
let logo_data = @src.generate_logo_data(sequences)
// logo_data = [(0, "A", 0.75, 0.79), (1, "C", 1.0, 2.0), ...]

// 模体富集分析
let motif_hits = @src.motif_enrichment(
  sequences=["ACGTACGT", "TCCGACGT", "ACGTACGT"],
  background=["AAAAAAAAAAAAAAAA", "TTTTTTTTTTTTTTTT"]
)

// Pearson 相关性比较两个模体
let motif2 = @src.create_motif(name="motif2", ...)
let correlation = @src.motif_correlation(motif, motif2)
```
...
```moonbit
// 加载 PDB 结构
let structure = @src.load_pdb("1CRN.pdb")
let model = structure.find_model(0).unwrap()
let chain = model.find_chain('A').unwrap()

// 计算 SASA (溶剂可及表面积)
let sasa = @src.calc_sasa(chain, probe_radius=1.4, n_points=100)
// sasa = [("CA", 12.5), ("CB", 8.3), ...]

// 计算总 SASA
let total_sasa = @src.calc_total_sasa(chain)
// total_sasa = 1234.5

// Ramachandran 图数据
let rama_data = @src.ramachandran_plot(chain)
// rama_data = [(phi, psi, residue_name), ...]

// 结构质量评估
let quality = @src.classify_ramachandran(rama_data)
// quality = {core: 85, allowed: 10, generously_allowed: 3, disallowed: 2}

// Kyte-Doolittle 疏水性分析
let hydrophobicity = @src.get_hydrophobicity("A")
// hydrophobicity = 1.8 (Ala)

// 序列属性距离矩阵
let seq_props = @src.sequence_property_matrix(chain, property_fn=@src.get_hydrophobicity)
// seq_props = [[0.0, 0.5, 1.2], [0.5, 0.0, 0.8], ...]
```
...
```bash
# 运行 SeqUtils 高级功能示例
moon run examples/seq_complexity_demo/main.mbt

# 运行 Motifs 高级功能示例
moon run examples/motifs_demo/main.mbt

# 运行 PDB 结构分析高级功能示例
moon run examples/pdb_analysis_demo/main.mbt

# 运行 EMBOSS 接口示例 (包含 GC/AT 偏斜和距离计算)
moon run examples/emboss_demo/main.mbt
```
